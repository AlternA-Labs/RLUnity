...
public class RocketAgent : Agent
{
    ...
    [SerializeField] private bool AstroDestroyed = false;
    ...
    public override void OnEpisodeBegin()
    {
        ...
        */
        ...
        if (GameObject.FindWithTag("Astro") != null)
        {
            previousDistanceToAstro = Vector3.Distance(transform.localPosition, astro.localPosition);
        }
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        */
        ...
        if (GameObject.FindWithTag("Astro") != null)
        {
            ...
            float distanceDelta = previousDistanceToAstro - currentDistance;  // + ise yaklaştık, - ise uzaklaştık
           // Debug.Log($"distanceDelta: {distanceDelta}");
            AddReward(distanceDelta * 5 * approachRewardFactor);

            previousDistanceToAstro = currentDistance;
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.TryGetComponent<landzone>(out landzone goal))
        {
            Debug.Log("touchdown");
            ...
            if (AstroDestroyed && rotx >= -2.5f && rotx <= 2.5f && roty >= 177.5f && roty <= 182.5f && rotz >= -2.5f && rotz <= 2.5f)
            {
                AddReward(1000f);
                Debug.Log("Success");
                EndEpisode();    
